...
import type { OrganizationUpdate } from '@/store/entities/organization/model/type'
...
interface OrganizationUpdatesManagementProps {
	organizationId: number
}

export function OrganizationUpdatesManagement({
	organizationId,
}: OrganizationUpdatesManagementProps) {
	...
	const {
		...
		isLoading,
		refetch,
	} = useGetOrganizationUpdatesQuery(organizationId)
...
function UpdateCard({
	...
	const [deleteOrganizationUpdate, { isLoading: isDeleting }] =
		useDeleteOrganizationUpdateMutation()
	const [showDeleteDialog, setShowDeleteDialog] = useState(false)

	const handleDelete = async () => {
		try {
			await deleteOrganizationUpdate(update.id).unwrap()
			toast.success('Обновление успешно удалено')
			setShowDeleteDialog(false)
			onDelete()
		} catch (error) {
			logger.error('Error deleting organization update:', error)
