...
export function QuestDetails({
	...
	const { user, participateInQuest, contributeToQuest, checkAndUnlockAchievements } =
		useUser()
	...
	const handleRoleSelect = (role: Parameters<typeof participateInQuest>[1]) => {
		if (quest) {
			...
			addNotification({
				...
				message: `Вы успешно присоединились к квесту "${quest.title}" в роли ${role === 'financial' ? 'Финансового воина' : role === 'volunteer' ? 'Волонтера-героя' : 'Амбассадора'}`,
				questId: quest.id,
				icon: '🎯',
			})
	...
	const handleDonate = (amount: number, stageId: string) => {
		if (quest) {
			contributeToQuest({
				...
				impact: `Внесли ${formatCurrency(amount)} на этап "${quest.stages.find(s => s.id === stageId)?.title}"`,
			})
			checkAndUnlockAchievements(quest.id)
			addNotification({
				type: 'donation_received',
				title: 'Спасибо за поддержку!',
				message: `Вы внесли ${formatCurrency(amount)} на этап "${quest.stages.find(s => s.id === stageId)?.title}"`,
				questId: quest.id,
				stageId,
				icon: '💰',
			})
	...
	const handleVolunteerRegister = (
		...
		addNotification({
			...
			message: `Вы зарегистрировались на событие "${quest?.stages.find(s => s.id === stageId)?.title}"`,
			questId: quest!.id,
			stageId,
			icon: '👷',
		})
	...
	return (
		<>
			...
			<section
				className={`fixed left-5 top-[88px] bottom-20 w-[480px] max-w-[calc(100vw-40px)] z-1000 bg-white/98 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/80 overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] ${
													...
													<div className='flex items-center justify-between'>
														<div className='text-sm'>
															...
															<span className='text-slate-600'>
																{formatCurrency(stage.requirements.financial.collected, stage.requirements.financial.currency)} из{' '}
																{formatCurrency(stage.requirements.financial.needed, stage.requirements.financial.currency)}
															</span>
														</div>
														{isParticipating &&
													...
													<div className='flex items-center justify-between'>
														...
														{isParticipating &&
															...
															stage.status !== 'completed' && (
																<Button
																	size='sm'
																	onClick={() => setShowVolunteerRegistration({ stage })}
